...
export default function VoiceNoteScreen() {
  ...
  const toggleRecording = async () => {
    if (recorderState.isRecording) {
      ...
      notify("Voice note captured locally. Confirm the operational summary before recording the event.", "success");
      return;
    }
    ...
    try {
      ...
      notify("Atlas could not start this recording. Check the microphone permission and try again.", "warning");
    }
  };
  ...
  const recordEvent = async () => {
    ...
    await enqueue({ eventType: "issue_reported", entityType: "issue", entityId: "voice-field-report", payload: { summary: summary.trim(), audioCaptured: true, localAudioUri: uri, capture: "native_microphone", storageState: "local_pending_upload" } });
    createTask("Review voice field report", "Voice-reported issue");
    notify(online ? "Voice field report recorded with a linked operational event." : "Voice field report is queued safely until connectivity returns.", "success");
    router.back();
  };
  ...
  return <ScreenContainer className="flex-1" edges={["top", "left", "right", "bottom"]} containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    ...
    <Surface style={styles.noteCard}><Text style={styles.noteLabel}>OPERATIONAL SUMMARY</Text><Text style={styles.noteHint}>Describe the observable signal without guessing at the cause. The original recording remains local until an object-storage upload path is configured.</Text><TextInput value={summary} onChangeText={setSummary} placeholder="Example: Machine 14 is overheating and a coolant hose appears loose." placeholderTextColor="#70858C" multiline style={styles.input} textAlignVertical="top" /></Surface>
    <Surface style={styles.guardrail}><Icon name="shield" color="#78B6FF" size={19} /><View style={styles.guardrailCopy}><Text style={styles.guardrailTitle}>Human review stays in the loop</Text><Text style={styles.guardrailText}>Atlas records the signal and creates follow-up work. It does not diagnose equipment or initiate irreversible action from a voice note alone.</Text></View></Surface>
...
const styles = StyleSheet.create({
  ...
  title: { color: "#E8F0F1", fontSize: 22, fontWeight: "800", letterSpacing: -0.6, marginTop: 2 },
  intro: { alignItems: "flex-start", flexDirection: "row", gap: 11, padding: 14 },
  introCopy: { flex: 1, gap: 3 },
  introTitle: { color: "#E8F0F1", fontSize: 15, fontWeight: "800" },
  introText: { color: "#9DAFB4", fontSize: 12, lineHeight: 18 },
  recorderWrap: { alignItems: "center", gap: 8, paddingVertical: 11 },
  recordButton: { alignItems: "center", backgroundColor: "#21D4C2", borderColor: "#76F4E6", borderRadius: 56, borderWidth: 2, height: 104, justifyContent: "center", shadowColor: "#000", shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.32, shadowRadius: 8, width: 104 },
  recording: { backgroundColor: "#C6473D", borderColor: "#FF978A" },
  recordState: { color: "#D6E3E5", fontSize: 13, fontWeight: "800", marginTop: 3 },
  timer: { color: "#789099", fontSize: 17, fontWeight: "800", letterSpacing: 1.2 },
  noteCard: { gap: 8, padding: 13 },
  noteLabel: { color: "#7F949B", fontSize: 10, fontWeight: "900", letterSpacing: 0.8 },
  noteHint: { color: "#98AAAF", fontSize: 11, lineHeight: 16 },
  input: { backgroundColor: "#17242A", borderColor: "#2A3C42", borderRadius: 11, borderWidth: 1, color: "#E8F0F1", fontSize: 13, minHeight: 105, padding: 11 },
  guardrail: { alignItems: "flex-start", flexDirection: "row", gap: 10, padding: 13 },
  guardrailCopy: { flex: 1, gap: 3 },
  guardrailTitle: { color: "#D9E6E8", fontSize: 12, fontWeight: "800" },
  guardrailText: { color: "#8FA4AA", fontSize: 11, lineHeight: 16 },
});
