...
import { orders, supplierCommitments } from "@/lib/atlas-data";
...
export default function CommerceScreen() {
  ...
  return <ScreenContainer className="flex-1" edges={["top", "left", "right", "bottom"]} containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <SectionTitle eyebrow="Supply chain" title="Supplier commitments" />
    <View style={styles.orderList}>{supplierCommitments.map((supplier) => <Surface key={supplier.id} style={styles.orderCard}><View style={styles.orderTop}><Text style={styles.orderId}>{supplier.id}</Text><SeverityPill severity={supplier.severity} label={supplier.status} /></View><Text style={styles.customer}>{supplier.name}</Text><Text style={styles.product}>{supplier.commitment}</Text><Text style={styles.note}>{supplier.detail}</Text><PrimaryButton label="Open linked materials" kind="secondary" icon="inventory" onPress={() => router.push("/operations")} /></Surface>)}</View>
