...
export default function InspectionScreen() {
  ...
  const recordInspection = async () => {
    ...
    await enqueue({ eventType: "inspection_recorded", entityType: "inspection", entityId: "MX-14-cooling-loop", facilityId: undefined, payload: { assetCode: "AT-PL-014", checklist: complete, evidence: evidence.trim(), result: "recorded", source: "guided_inspection" } });
    createTask("Review Machine 14 inspection evidence", "Verification follow-up");
    notify(online ? "Inspection evidence recorded and queued for live verification." : "Inspection evidence is safely queued until connectivity returns.", "success");
    router.back();
  };
  ...
  return <ScreenContainer className="flex-1" edges={["top", "left", "right", "bottom"]} containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
    ...
    <Surface style={styles.syncCard}><Icon name="cloud-queue" color={online ? "#21D4C2" : "#F5B84B"} size={19} /><View style={styles.syncCopy}><Text style={styles.syncTitle}>{online ? "Event synchronization ready" : "Offline-first inspection"}</Text><Text style={styles.syncText}>{queue.length ? `${queue.length} operational event${queue.length === 1 ? " is" : "s are"} awaiting sync.` : online ? "The verified inspection will synchronize after it is recorded." : "This inspection will remain durable on the device until connectivity returns."}</Text></View></Surface>
...
const styles = StyleSheet.create({
  ...
  context: { alignItems: "flex-start", flexDirection: "row", gap: 11, padding: 14 },
  contextIcon: { alignItems: "center", backgroundColor: "#183536", borderRadius: 12, height: 43, justifyContent: "center", width: 43 },
  contextCopy: { flex: 1, gap: 3 },
  contextLabel: { color: "#789099", fontSize: 9, fontWeight: "900", letterSpacing: 0.7 },
  contextTitle: { color: "#E8F0F1", fontSize: 15, fontWeight: "800" },
  contextBody: { color: "#9EB1B6", fontSize: 11, lineHeight: 16, marginTop: 2 },
  sectionEyebrow: { color: "#7F949B", fontSize: 10, fontWeight: "900", letterSpacing: 0.9, marginTop: 2 },
  steps: { gap: 9 },
  step: { alignItems: "flex-start", backgroundColor: "#142126", borderColor: "#2A3A40", borderRadius: 14, borderWidth: 1, flexDirection: "row", gap: 11, padding: 13 },
  stepComplete: { backgroundColor: "#173334", borderColor: "#3D7773" },
  check: { alignItems: "center", backgroundColor: "#1D3036", borderRadius: 14, height: 28, justifyContent: "center", width: 28 },
  checkComplete: { backgroundColor: "#21D4C2" },
  stepCopy: { flex: 1, gap: 3 },
  stepIndex: { color: "#789099", fontSize: 9, fontWeight: "900", letterSpacing: 0.6 },
  stepTitle: { color: "#DCE7E9", fontSize: 13, fontWeight: "800" },
  stepDetail: { color: "#98AAAE", fontSize: 11, lineHeight: 16 },
  evidenceCard: { gap: 8, padding: 13 },
  evidenceHint: { color: "#95A8AD", fontSize: 11, lineHeight: 16 },
  input: { backgroundColor: "#17242A", borderColor: "#2A3C42", borderRadius: 11, borderWidth: 1, color: "#E8F0F1", fontSize: 13, minHeight: 98, padding: 11 },
  syncCard: { alignItems: "flex-start", flexDirection: "row", gap: 10, padding: 13 },
  syncCopy: { flex: 1, gap: 2 },
  syncTitle: { color: "#DCE8E9", fontSize: 12, fontWeight: "800" },
  syncText: { color: "#8FA4AA", fontSize: 11, lineHeight: 16 },
});
