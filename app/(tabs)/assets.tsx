...
export default function AssetsScreen() {
  ...
  const visibleAssets = useMemo(() => assets.filter((asset) => `${asset.name} ${asset.code} ${asset.location}`.toLowerCase().includes(query.toLowerCase())), [query]);
  return <ScreenContainer className="flex-1" containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <View style={styles.header}><View><Text style={styles.eyebrow}>Physical world registry</Text><Text style={styles.title}>Assets</Text></View><IconAction name="qr-code-scanner" label="Scan a code" accent onPress={() => router.push({ pathname: "/asset/[id]", params: { id: "MX-14" } })} /></View>
    ...
    <SectionTitle eyebrow="Operational identity" title={`${visibleAssets.length} assets in context`} />
