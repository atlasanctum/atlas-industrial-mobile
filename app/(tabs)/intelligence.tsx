...
export default function IntelligenceScreen() {
  ...
  return <ScreenContainer className="flex-1" containerClassName="bg-background"><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <SectionTitle eyebrow="Advanced operating layer" title="Contextual intelligence" />
    <View style={styles.evidence}><LinkedRecord icon="account-tree" title="Scenario engine" detail="Compare recovery paths, assumptions, impact, and approval requirements." onPress={() => router.push("/scenario")} /><LinkedRecord icon="smart-toy" title="Agent coordination" detail="Synthesize operations, asset, supply, logistics, and finance evidence." onPress={() => router.push("/agents")} /><LinkedRecord icon="memory" title="Industrial memory" detail="Turn event, decision, outcome, and lesson into institutional learning." onPress={() => router.push("/memory")} /><LinkedRecord icon="gpp-good" title="Governance center" detail="Inspect policy thresholds, approval authority, and audit invariants." onPress={() => router.push("/governance")} /></View>
